...
Lexer.prototype.token = function(src, top, isRoot) {
  src = src.replace(/^ +$/gm, '');

  var totalLinesCount = 0;
  var nextSourceLine = 0;
  if (isRoot) {
...
Renderer.prototype.code = function(code, infostring, escaped, line) {
  ...
  if (!lang) {
    return '<pre source-line="' + line + '"><code>'
      + (escaped ? code : escape(code, true))
      + '</code></pre>';
  }

  return '<pre source-line="' + line + '"><code class="'
...
Renderer.prototype.heading = function(text, level, raw, slugger, line) {
  ...
  // ignore IDs
  return '<h' + level + ' source-line="' + line + '">' + text + '</h' + level + '>\n';
};
...
Renderer.prototype.list = function(body, ordered, start, line) {
  var type = ordered ? 'ol' : 'ul',
      startatt = (ordered && start !== 1) ? (' start="' + start + '"') : '';
  return '<' + type + startatt + ' source-line="' + line + '">\n' + body + '</' + type + '>\n';
};
...
Parser.prototype.tok = function() {
  switch (this.token.type) {
    ...
    case 'list_start': {
      ...
      return this.renderer.list(body, ordered, start, this.token.line);
    }
